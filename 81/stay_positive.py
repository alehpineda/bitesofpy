...
def filter_tweets_on_polarity(tweets, keep_positive=True):
    ...
    return [tweet for tweet in tweets
            if _condition(tweet.polarity, keep_positive)]


def _condition(polarity, keep_positive):
    return polarity > 0 if keep_positive else polarity < 0
...
def order_tweets_by_polarity(tweets, positive_highest=True):
    ...
    return sorted(tweets, key=lambda tweet: tweet.polarity, reverse=reverse)
