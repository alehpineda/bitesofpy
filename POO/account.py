...
class Account:
    ...
    # Transaction managment with magic methods
    ...
    def __str__(self):
        'Nice class reporting when doing atr(acc_instance)'
        tt = ['- {t}' for t in self._transactions]
        s = [
            'Account of {self.owner}',
            'Start Balance: {self.start_balance}',
            'Transactions:',
            '\n'.join(tt) or 'None',
            'End Balance: {self.balance}'
        ]
        return '\n\n'.join(s)
