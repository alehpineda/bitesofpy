"""
In this Bite you will use ElementTree to parse some Nolan movies we 
extracted from OMDb.

Luckily most APIs switched to JSON, but sometimes XML is all there is, 
so at least learn to parse some! Complete the get_tree, get_movies and 
get_movie_longest_runtime functions below. See the docstrings and tests 
for more info.
...
def get_movie_longest_runtime():
    ...
    _get_runtime = max([child.attrib['runtime'] for child in get_tree()])
    for child in get_tree():
        if child.attrib['runtime'] == _get_runtime:
...
def _get_runtime1(movie):
    """Helper function to extract the minutes (int) from the runtime movie attribute"""
    return int(movie.attrib['runtime'].rstrip(' min'))


def get_movie_longest_runtime1():
    """Call get_tree again and return the movie with the longest runtime in minutes,
       for latter consider adding a _get_runtime helper"""
    tree = get_tree()
    movies = [(movie.attrib['title'], _get_runtime1(movie))
              for movie in tree.iter(tag='movie')]
    max_movie, _ = max(movies, key=lambda m: m[1])
    return max_movie
